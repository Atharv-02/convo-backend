...
exports.updateStudentPaymentStatus = asyncHandler(async (req, res, next) => {
  ...
  await student.save();
  if (day === '') {
    const message = `You have selected the option to collect the degree certificate through courier.`;
    await sendEmail({
      email: student.email,
    ...
    const message = `You have selected the option to collect the degree certificate in person on ${date} 2023 along with ${student.companions} companions. Thereby your registration for 10th MUJ Convocation is successful. Please do not login again.`;
    await sendEmail({
      email: student.email,
      subject: 'Registration Successful',
      message: message,
